...
class CachedFeatureLoader:
        ...
        if shots != -1:
            ...
            for c in classes:
                idx_selected = np.random.choice(np.where(labels == c)[0],
                                            min(shots, class_counts[c]),
                                            replace=False
                                            )
                if shots < class_counts[c]:
                    class_counts[c] -= shots
            ...
            selected_elements_list = np.where(np.isin(labels.cpu().numpy(), classes))[0].tolist()

        all_selected_features = features[selected_elements_list,:]
        all_selected_labels = labels[selected_elements_list].clone()
    ...
    def load_train_data(self, shots: int, n_classes: int):
        """
        ...
        train_features, train_labels, class_mapping = self._subsample_shots(
