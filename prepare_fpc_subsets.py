...
def main():
    ...
    parser.add_argument("--feature_extractor", choices=["vit-b-16", "BiT-M-R50x1"],
                        default="vit-b-16", help="Feature extractor used for cached features.")
    parser.add_argument("--training_sample_size", type=int, default=1000,
                        help="Per-model training-set size N.")
    parser.add_argument("--frame_size_ratios", type=float, nargs="+",
                        default=[0.25, 0.5, 0.75],
                        help="Values of N / N_plus.")
    parser.add_argument("--eval_sample_size", type=int, default=50,
                        help="Number of fixed evaluation samples.")
    ...
    # Population pool: samples completely outside the maximum finite frame.
    mask = np.ones(n_total, dtype=bool)
    mask[X_superset_max_indices] = False
    X_population_indices = np.flatnonzero(mask)
